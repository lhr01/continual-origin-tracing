...
class BaseLearner(object):
    ...
    def tsne(self, showcenters=False, Normalize=False):
        ...
        test_dataset = self.data_manager.get_texts_dataset(np.arange(0, tot_classes), source='test', mode='test')
        ...
        scatter=plt.scatter(embedding[:,0],embedding[:,1],c=y_true,s=20,cmap=plt.get_cmap("tab20"))

        plt.legend(*scatter.legend_elements())
        if showcenters:
            plt.scatter(clssscenters[:,0],clssscenters[:,1],marker='*',s=50,c=centerlabels,cmap=plt.get_cmap("tab20"),edgecolors='black')
        
        plt.savefig("./tsne/" + str(self.args['model_name'])+str(tot_classes)+'tsne.pdf')
        plt.close()
    ...
    def save_checkpoint(self, filename):
        self._network.cpu()
        save_dict = {
            "stages": self._cur_stage,
            "model_state_dict": self._network.state_dict(),
        }
        torch.save(save_dict, "{}_{}.pkl".format(filename, self._cur_stage))
    ...
    def _evaluate(self, y_pred, y_true):
        ...
        for cls in classes:
            ...
            # 初始化一个字典来存储被错误预测成其他类别的次数
            misclassified_counts = {}
            for other_cls in classes:
                misclassified_counts[other_cls] = np.sum(y_pred[class_indices] == other_cls) / class_total
            
            misclassifications[cls] = misclassified_counts
        
        ret["pre_class_accuracy"] = accuracies
        ret["misclassifications"] = misclassifications
        return ret

    def eval_task(self):
        self.topk = 1
        y_pred, y_true = self._eval_twice(self.test_loader)
        finall_accy = self._evaluate(y_pred, y_true)
    ...
    def _compute_accuracy(self, model, loader):
        ...
        for step, batch in enumerate(loader):
            ...
            with torch.no_grad():
                outputs, _ = model(inputs, self._cur_stage)
            predicts = torch.max(outputs, dim=1)[1]
            correct += (predicts.cpu() == inputs['labels'].cpu()).sum()
            total += len(inputs['labels'])

        return np.around(tensor2numpy(correct) * 100 / total, decimals=2)

    def _eval_twice(self, loader):
        self._network.eval()
        y_pred, y_true = [], []
        for step, batch in enumerate(loader):
            batch = tuple(t.to(self._device) for t in batch)
            inputs = {
                "input_ids": batch[0],
                "attention_mask": batch[1],
                "labels": batch[2]  # 对于 LLaMA 没有 token_type_ids
            }
            with torch.no_grad():
                _, outputs = self._network(inputs, self._cur_stage)
            y_pred.append(np.array(outputs['logits']))
    ...
    def _extract_vectors(self, loader):
        ...
        for step, batch in enumerate(loader):
            ...
            _targets = inputs['labels'].cpu().numpy()
            if isinstance(self._network, nn.DataParallel):
    ...
    def _reduce_exemplar(self, data_manager, m):
        ...
        for class_idx in range(self._known_classes):
            ...
            # Exemplar mean
            idx_dataset = data_manager.get_texts_old_dataset(
                [],
                source="train",
                mode="test",
                appendent=(dd, dt),
            )
            idx_old_loader = DataLoader(
                idx_dataset, batch_size=batch_size, shuffle=False
            )
            vectors, _ = self._extract_vectors(idx_old_loader)
            vectors = (vectors.T / (np.linalg.norm(vectors.T, axis=0) + EPSILON)).T
    ...
    def _construct_exemplar(self, data_manager, m):
        ...
        for class_idx in range(self._known_classes, self._total_classes):
            data, targets, idx_dataset = data_manager.get_texts_dataset(
